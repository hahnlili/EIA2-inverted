"use strict";
let europeEmissions = 4209.3;
let europe2008 = 4965.7;
let africaEmissions = 1235.5;
let southAfricaEmissions = 1261.5;
let northAmericaEmissions = 6035.6;
let asiaEmissions = 16274.1;
let australiaEmissions = 2100.5;
let worldEmissions = europeEmissions + africaEmissions + southAfricaEmissions + northAmericaEmissions + asiaEmissions + australiaEmissions;
let satz1 = "Die Emission von Europa ist ";
satz1 += europeEmissions + " CO2.";
let prozent = " 4%";
let satz2 = " Relativ zur Gesamtemission der Welt verursacht Europe damit";
satz2 += prozent;
let differenzEuropa = europeEmissions;
differenzEuropa = europeEmissions - europe2008;
let satz3 = " Für Europa hat sich 2018 im Vergleich zu 2008 die Emission um 1,2%. verändert. ";
let satz4 = "2018 im Vergleich zu 2008 sind das -400 kg CO2.";
window.addEventListener('load', init);
function init() {
    document.querySelector("#europa").addEventListener('click', Europa);
    document.querySelector("#asia").addEventListener('click', Asien);
    document.querySelector("#sa").addEventListener('click', SuedAmerika);
    document.querySelector("#na").addEventListener('click', NordAmerika);
    document.querySelector("#a").addEventListener('click', Afrika);
    document.querySelector("#aus").addEventListener('click', Australien);
}
let infos = document.querySelector("#info1");
function Europa() {
    console.log("Emissionen aus Europa");
    let Europa = document.querySelector("title");
    Europa.innerHTML = "Emissionen in Europa:";
    let en = document.querySelector("h1");
    en.innerHTML = "Emissionen in Europa:";
}
function Asien() {
    console.log("Emissionen aus Asien");
    let Asien = document.querySelector("title");
    Asien.innerHTML = "Emissionen in Asien:";
    let eu = document.querySelector("h1");
    eu.innerHTML = "Emissionen in Asien:";
    infos.innerHTML = "12954.7";
}
function SuedAmerika() {
    console.log("Emissionen aus Südamerika");
    let suedamerika = document.querySelector("title");
    suedamerika.innerHTML = "Emissionen in Südamerika:";
    let ea = document.querySelector("h1");
    ea.innerHTML = "Emissionen in Südamerika:";
}
function NordAmerika() {
    console.log("Emissionen aus Nordamerika");
    let nordamerika = document.querySelector("title");
    nordamerika.innerHTML = "Emissionen in Nordamerika:";
    let ee = document.querySelector("h1");
    ee.innerHTML = "Emissionen in Nordamerika:";
}
function Afrika() {
    console.log("Emissionen aus Afrika");
    let afrika = document.querySelector("title");
    afrika.innerHTML = "Emissionen in Afrika:";
    let ei = document.querySelector("h1");
    ei.innerHTML = "Emissionen in Afrika:";
}
function Australien() {
    console.log("Emissionen aus Australien");
    let australien = document.querySelector("title");
    australien.innerHTML = "Emissionen in Australien:";
    let ee = document.querySelector("h1");
    ee.innerHTML = "Emissionen in Australien:";
}
console.log(satz1 + satz2 + satz3 + satz4);
console.log(africaEmissions, southAfricaEmissions, northAmericaEmissions, asiaEmissions, australiaEmissions);
//# sourceMappingURL=index.js.map